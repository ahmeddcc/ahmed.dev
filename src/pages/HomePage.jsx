...
export function HomePage() {
  ...
  return (
    <PageLayout>
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center w-full">
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="space-y-8">
            ...
            <motion.div variants={fadeUp}>
              <h1 className="text-5xl md:text-6xl lg:text-7xl font-bold font-display text-[#f0f4ff] leading-[1.05] tracking-tight">
                {personal.name || 'Ahmed Hassan'}
              </h1>
              <div className="mt-4 h-12 flex items-center">
                <span className="text-2xl md:text-3xl font-display text-transparent bg-clip-text bg-gradient-to-r from-[#63d3bf] to-[#a78bfa]">
                  {displayed}<span className="animate-pulse">|</span>
                </span>
              </div>
            </motion.div>

            <motion.p variants={fadeUp} className="text-[#8892a4] text-lg leading-relaxed max-w-lg">
          ...
          >
            <div className="relative w-80 h-80">
              ...
              <div className="relative w-full h-full rounded-full border border-[rgba(99,211,191,0.15)] flex items-center justify-center overflow-hidden bg-[#111827]">
                {personal.avatar ? (
                  <img src={personal.avatar} alt={personal.name} className="w-full h-full object-cover" />
                ) : (
                  <div className="text-8xl font-bold font-display gradient-text">
              ...
              <motion.div animate={{ y: [0, -8, 0] }} transition={{ repeat: Infinity, duration: 3, ease: 'easeInOut' }}
                className="absolute -top-4 -right-4 glass rounded-xl px-3 py-2 text-xs font-mono text-[#63d3bf] border border-[rgba(99,211,191,0.2)]">
                React ⚛️
              </motion.div>
              <motion.div animate={{ y: [0, 8, 0] }} transition={{ repeat: Infinity, duration: 3.5, ease: 'easeInOut', delay: 0.5 }}
                className="absolute -bottom-4 -left-4 glass rounded-xl px-3 py-2 text-xs font-mono text-[#a78bfa] border border-[rgba(167,139,250,0.2)]">
                Node.js 🟢
              </motion.div>
            </div>
          </motion.div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-[rgba(255,255,255,0.04)] rounded-2xl overflow-hidden border border-[rgba(255,255,255,0.05)]">
              {stats.map((stat, i) => (
                <div key={i} className="bg-[#080b14] px-6 py-8 text-center">
                  <p className="text-3xl md:text-4xl font-bold font-display gradient-text">{stat.value}</p>
                  <p className="text-sm text-[#8892a4] mt-2">{stat.label}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      <MotionSection>
        <div className="max-w-6xl mx-auto px-6 pb-20">
          <div className="rounded-2xl border border-[rgba(99,211,191,0.15)] bg-gradient-to-br from-[rgba(99,211,191,0.04)] to-[rgba(167,139,250,0.04)] p-10 text-center relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-br from-[#63d3bf]/5 to-[#a78bfa]/5" />
            <div className="relative">
              <h2 className="text-3xl font-bold font-display text-[#f0f4ff] mb-4">Ready to work together?</h2>
              <p className="text-[#8892a4] mb-8">Let's build something remarkable.</p>
              <Link to="/contact">
